...
executePlan gi g plan = (lg,dsl) where
  ...
  ((dsl,_),lg) = runState (runStateT lastNode CGA.empty) liftedGraph
...
constructNode :: Ord x => Plan x -> CoveringNode
                          -> StateT (CoveringGraphAnnotation Int)
                               (State (CombinationGraph x)) Int
constructNode plan coveringNode = let
    lookupNodeWrapper otherAction = do
      constructed <- get
      case CGA.lookup coveringNode constructed of
        Just n  -> return n
        Nothing -> otherAction
    myCycleList = cycleOfNode coveringNode
    myIndex = 0
    planOfNode cn = CGA.forceLookup cn plan
  ...
  in lookupNodeWrapper $ do
    ...
    constructed <- get
    put $ V.ifoldl (\m i node -> CGA.insert (myCycle V.! i) node m)
